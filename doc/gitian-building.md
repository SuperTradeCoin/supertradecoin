...
*Setup instructions for a gitian build of MineGoldCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MineGoldCoin
...
- [Building MineGoldCoin](#building-minegoldcoin)
...
Clone the git repositories for minegoldcoin and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/minegoldcoin/minegoldcoin
```
...
In [doc/release-process.md](release-process.md) in the minegoldcoin repository under 'Fetch and build inputs'.
...
Building MineGoldCoin
----------------

To build MineGoldCoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the minegoldcoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/minegoldcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/minegoldcoin/minegoldcoin
...
For example:
```bash
URL=https://github.com/langerhans/minegoldcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit minegoldcoin=${COMMIT} --url minegoldcoin=${URL} ../minegoldcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit minegoldcoin=${COMMIT} --url minegoldcoin=${URL} ../minegoldcoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/minegoldcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/minegoldcoin-build.assert
```
...
[minegoldcoin/gitian.sigs](https://github.com/minegoldcoin/gitian.sigs/) repository, or
