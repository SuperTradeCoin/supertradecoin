...
#if defined(NDEBUG)
# error "MineGoldCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "MineGoldCoin Signed Message:\n";
...
//TODO: this isn't identical to minegoldcoin reference client.
...
int64_t GetBlockValue(int nHeight, int64_t nFees, uint256 prevHash)
{
  ...
  if (nHeight == 0) {
      ...
      nSubsidy >>= ((nHeight - 1600000 + 400000) / 1000);
    }
    else
    {
        nSubsidy = 25 * COIN;
...
static const int64_t nTargetTimespan = 10 * 30; // MineGoldCoin: every 5min
static const int64_t nTargetTimespanNEW = 30 ; // MineGoldCoin: every 30sec
static const int64_t nTargetSpacing = 30; // MineGoldCoin: 30sec 
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MineGoldCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("minegoldcoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in MineGoldCoin
...
bool AcceptBlockHeader(CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        // MineGoldCoin: reject v2 and v1 blocks at the same time, only check once
        if (block.GetBaseVersion() < 3) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    // MineGoldCoin: reject ONLY if block.nVersion=3 has a supermajority because CBlockIndex::IsSuperMajority
