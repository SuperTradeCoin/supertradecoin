...
#if defined(NDEBUG)
# error "SuperTradeCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "SuperTradeCoin Signed Message:\n";
...
//TODO: this isn't identical to supertradecoin reference client.
...
int64_t GetBlockValue(int nHeight, int64_t nFees, uint256 prevHash)
{
  ...
  if (nHeight == 0) {
      ...
      nSubsidy >>= ((nHeight - 1200000 + 200000) / 1000);
    }
    else
    {
        nSubsidy = 50 * COIN;
...
static const int64_t nTargetTimespan = 10 * 30; // SuperTradeCoin: every 5min
static const int64_t nTargetTimespanNEW = 30 ; // SuperTradeCoin: every 30sec
static const int64_t nTargetSpacing = 30; // SuperTradeCoin: 30sec 
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SuperTradeCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("supertradecoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in SuperTradeCoin
...
bool AcceptBlockHeader(CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        // SuperTradeCoin: reject v2 and v1 blocks at the same time, only check once
        if (block.GetBaseVersion() < 3) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    // SuperTradeCoin: reject ONLY if block.nVersion=3 has a supermajority because CBlockIndex::IsSuperMajority
