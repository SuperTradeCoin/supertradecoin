UNIX BUILD NOTES
====================
Some notes on how to build SuperTradeCoin in Unix. 
...
This will build supertradecoin-qt as well if the dependencies are met.
...
memory available when compiling SuperTradeCoin Core. With 512MB of memory or less
...
If you want to build SuperTradeCoin-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a supertradecoin-qt executable will be
built by default.

Notes
-----
The release is built with GCC and then "strip supertradecoind" to strip the debug
...
# Pick some path to install BDB to, here we create a directory within the supertradecoin directory
...
# Configure SuperTradeCoin Core to use our own-built instance of BDB
cd $BITCOIN_ROOT
...
cryptography functions, and therefore cannot be used for SuperTradeCoin (or other
...
Security
--------
To help make your supertradecoin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, supertradecoin should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, supertradecoin may be compiled in
